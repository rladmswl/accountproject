  ...
  // JTextField idt = new JTextField(7);
   ...
   ActionListener save = new ActionListener() { // 파일 - 저장
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         int ret = chooser.showSaveDialog(SiteInfo.this);
         if (ret == JFileChooser.APPROVE_OPTION) { // 저장 버튼 누른 경우
            String fileName = chooser.getSelectedFile().getName(); // 파일 이름
            JOptionPane.showMessageDialog(SiteInfo.this, fileName, "확인", JOptionPane.INFORMATION_MESSAGE);
         }
         else if (ret == JFileChooser.CANCEL_OPTION) { // 취소 버튼 누른 경우
            JOptionPane.showMessageDialog(SiteInfo.this, "파일을 선택하지 않았습니다.", "경고", JOptionPane.WARNING_MESSAGE);
            return;
         }
      }
   };
...
/*
   class SiteDetailInfo extends SiteInfo{
